...
void writeVTU(const int frame, const std::vector<Particle<2>>& particles)
{
	...
	// --- Points (must be 3 components in VTK) ---
	out << "      <Points>\n";
	out << "        <DataArray type=\"Float32\" NumberOfComponents=\"3\" format=\"ascii\">\n";
	for (const auto& p : particles)
		out << p.Position.x << " " << p.Position.y << " 0\n";
	...
	// Velocity (exported as 3D vector with z = 0)
	out << "        <DataArray type=\"Float32\" Name=\"velocity\" NumberOfComponents=\"3\" format=\"ascii\">\n";
	for (const auto& p : particles)
		out << p.Velocity.x << " " << p.Velocity.y << " 0\n";
	out << "        </DataArray>\n";
...
void writeVTUBinary(const int frame, const std::vector<Particle<2>>& particles)
{
	...
	// --- Compute byte sizes (still 3 components for VTK compatibility) ---
	uint32_t pointsSize = N * 3 * sizeof(float);
	uint32_t densitySize = N * sizeof(float);
	uint32_t pressureSize = N * sizeof(float);
	uint32_t velocitySize = N * 3 * sizeof(float);
