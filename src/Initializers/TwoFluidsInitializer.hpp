...
class TwoFluidsInitializer;

template <ParticleSet<2> Particles>
class TwoFluidsInitializer<2, Particles> : public SimInitializer<2, Particles>
{
public:
...
template <ParticleSet<2> Particles>
inline void TwoFluidsInitializer<2, Particles>::Init(Particles& out, float h, Objects& obj) const
{
	...
	for (float x = 25; x < 55; x += 0.25)
	{
		for (float y = 10; y < 30; y += 0.25)
		{
			Particle<2> p;
			p.Type = FLUID;
	...
	for (float x = 25; x < 55; x += 0.5)
	{
		for (float y = 40; y < 50; y += 0.5)
		{
			Particle<2> p;
			p.Type = FLUID;
			p.Position.y = float(y) / maxy;
...
template <ParticleSet<2> Particles>
inline std::pair<coord<float, 2>, coord<float, 2>> TwoFluidsInitializer<2, Particles>::GetDomain() const
{
