...
namespace cudasph
{
		...
		Grid2& grid, GridGPU& gridGPU)
	{
		...
		}
		{
			...
			if (frame == 0)
			{
				ComputeDensityOrBoundaryPsiKernel2<<<blocks, threads>>>();
				err = cudaDeviceSynchronize();
				if (err != cudaSuccess)
			...
			UpdateVelocityPositionInitialKernel2<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("UpdateVelocityPositionInitialKernel2: %s\n", cudaGetErrorString(err));
...

		{
			...
			LOG("BANANA 4\n");
			ComputeDensityAndPressureKernel2<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("ComputeDensityAndPressureKernel2: %s\n", cudaGetErrorString(err));

			ComputeAccelerationPressureKernel2<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("ComputeAccelerationPressureKernel2: %s\n", cudaGetErrorString(err));

			UpdateVelocityPositionIterationKernel2<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("UpdateVelocityPositionIterationKernel2: %s\n", cudaGetErrorString(err));
		...
		Grid3& grid, GridGPU& gridGPU)
	{
		...
		if (frame == 0)
		{
			...
			cudaMemcpyToSymbol(DParticles3, &dParticles, sizeof(ParticlesCuda<3>));
			cudaMemcpyToSymbol(DParams, &params, sizeof(base::SPHParams));
			cudaMemcpyToSymbol(DSL2, &sl2, sizeof(float));
		...
		}
		{
			...
			if (frame == 0)
			{
				ComputeDensityOrBoundaryPsiKernel3<<<blocks, threads>>>();
				err = cudaDeviceSynchronize();
				if (err != cudaSuccess)
					printf("ComputeDensityOrBoundaryPsiKernel3: %s\n", cudaGetErrorString(err));

				ComputeAccelerationViscosityKernel3<<<blocks, threads>>>();
				err = cudaDeviceSynchronize();
				if (err != cudaSuccess)
					printf("ComputeAccelerationViscosityKernel3: %s\n", cudaGetErrorString(err));
			}

			UpdateVelocityPositionInitialKernel3<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("UpdateVelocityPositionInitialKernel3: %s\n", cudaGetErrorString(err));
...

		{
			...
			ComputeDensityAndPressureKernel3<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("ComputeDensityAndPressureKernel3: %s\n", cudaGetErrorString(err)); 
			
			ComputeAccelerationPressureKernel3<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("ComputeAccelerationPressureKernel3: %s\n", cudaGetErrorString(err)); 
			
			UpdateVelocityPositionIterationKernel3<<<blocks, threads>>>();
			err = cudaDeviceSynchronize();
			if (err != cudaSuccess)
				printf("UpdateVelocityPositionIterationKernel3: %s\n", cudaGetErrorString(err));
